...
class PosOrder(models.Model):
    ...
    def action_invoice_sent(self):
        ...
        xml_att = self.env['ir.attachment'].create({
            'name': self.fname_xml_comprobante,
            'type': 'binary',
        ...
        xml_res = self.env['ir.attachment'].create({
            'name': self.fname_xml_respuesta_tributacion,
            'type': 'binary',
    ...
    def _validahacienda_pos(self, max_orders=10, no_partner=True):
        ...
        for doc in pos_orders:
            ...
            if doc.state_tributacion == 'rejectedfixed':
                ...
                elif doc.tipo_documento == 'NC':
                    new_electronic_number = doc.session_id.config_id.NC_sequence_id.next_by_id()
                doc.message_post(
            ...
            if not doc.xml_comprobante or (doc.xml_comprobante and not doc.state_tributacion):
                if doc.xml_comprobante:
                    doc.xml_comprobante = False
                    doc.xml_respuesta_tributacion = False
